...
class Node:
    ...
    def set_state(self, state):
        possibleStates = [c.OPEN, c.CLOSED, c.BARRIER, c.START, c.END]
    ...
    def check_neighbours(self, grid):
        ...
        if (self.row - 1 < 0 and not grid[self.row - 1][self.col].is_state(c.BARRIER)): # Checks if row above (-y direction) is within our grid            
            self.neighbours.append(grid[self.row - 1][self.col])
        ...
        if (self.col - 1 < self.max_rows and not grid[self.row][self.col - 1].is_state(c.BARRIER)):  # Checks if col leftward (+x direction) is within our grid 
            self.neighbours.append(grid[self.row][self.col - 1])
